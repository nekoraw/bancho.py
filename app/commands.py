...
async def recent(ctx: Context) -> Optional[str]:
    ...
    if score.passed:
        ...
        if score.bmap.total_length != 0:
            ...
            l.append(f"FALHOU {{{completion * 100:.2f}% complete}})")
        else:
            l.append("FALHOU")
...
async def notes(ctx: Context) -> Optional[str]:
    ...
    if not res:
        return f"Nada notável encontrado relacionado a {target} nos últimos {days} dias."
...
async def unsilence(ctx: Context) -> Optional[str]:
    ...
    if len(ctx.args) < 2:
        return "Sintaxe inválida: !unsilence <nome> <motivo>"
...
async def shutdown(ctx: Context) -> Union[Optional[str], NoReturn]:
    ...
    if ctx.args:  # shutdown after a delay
        ...
        if delay < 15:
            return "Atraso minímo é de 15 segundos."
...
async def recalc(ctx: Context) -> Optional[str]:
    """Recalcula o pp."""
    return (
        "Por favor use tools/recalc.py ao invés desse comando.\n"
        "Discord oficial do osuAkatsuki/bancho.py em https://discord.gg/ShEQgUx."
    )
...
async def mp_scrim(ctx: Context, match: Match) -> Optional[str]:
    ...
    if len(ctx.args) != 1 or not r_match:
        return "Sintaxe inválida: !mp scrim <mdX onde X é o número da melhor de X>"
...
async def pool_add(ctx: Context) -> Optional[str]:
    ...
    if (mods, slot) in pool.maps:
        return f"{mods_slot} já está {pool.maps[(mods, slot)].embed}!"
